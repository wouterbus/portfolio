...
export default function FeaturedBox3({ onProjectClick }: FeaturedBox3Props) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const query = `*[_type == "project" && featured == true] | order(order asc, _createdAt desc) [2] {
          _id,
          title,
          heroBanner,
          thumbnail,
