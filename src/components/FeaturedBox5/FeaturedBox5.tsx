...
export default function FeaturedBox5({ onProjectClick }: FeaturedBox5Props) {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const query = `*[_type == "project" && featured == true] | order(order asc, _createdAt desc) [4] {
          _id,
          title,
          heroBanner,
          thumbnail,
